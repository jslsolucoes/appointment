...
public interface SchedulerSlotRepo extends CrudRepository<ScheduleSlot, Long> {
	...
	@Query("select sl from ScheduleSlot sl where sl.date between :min and :max and sl.id in (select sl2.id from ScheduleSlotTime slt inner join slt.scheduleSlot sl2 where slt.available = true)  ")
	public List<ScheduleSlot> dates(@Param("min") LocalDate min, @Param("max") LocalDate max);
